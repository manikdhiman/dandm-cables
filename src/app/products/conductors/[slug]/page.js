...
import { conductorsData } from "@/data/conductorsData";
import { ChevronRight, ArrowLeft, Send, X, CheckCircle2 } from "lucide-react";

export default function ConductorDetailPage() {
  const params = useParams();
  const slug = params?.slug;
  const product = conductorsData[slug];
    ...
    return (
      <main className="min-h-screen bg-brand-ice/20 py-24 px-6 text-center">
        <div className="max-w-md mx-auto bg-white p-8 rounded-3xl border border-gray-100 shadow-xl space-y-4">
          ...
          <p className="text-xs text-brand-gray">The requested conductor detail specification does not exist.</p>
          <Link href="/products/conductors" className="inline-block bg-brand-teal text-white text-xs font-bold px-6 py-3 rounded-xl">
            Back to Conductors
          </Link>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-brand-ice/20 pb-24 pt-28">
      <div className="max-w-6xl mx-auto px-6 space-y-12">
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-brand-gray">
          ...
          <Link href="/products/conductors" className="hover:text-brand-teal transition">Conductors</Link>
          <ChevronRight size={12} />
                ...
                <tbody>
                  {product.techSpecs.map((spec, idx) => (
                    <tr key={idx} className={idx % 2 === 0 ? "bg-brand-ice/30" : "bg-white"}>
                      ...
                      <td className="py-3.5 px-4 text-brand-gray border-b border-gray-100 font-medium">
                        {spec.value}
                      </td>
                    </tr>
                  ))}
              ...
              <>
                ...
                >
                  ...
                  <div>
                    ...
                    <textarea rows="3" placeholder="Specify conductor sizes, voltage parameters, or delivery timelines..." className="w-full p-3 rounded-xl border border-gray-200 focus:outline-none focus:border-brand-teal" />
                  </div>
